...
def main(args):
    ...
    data = data[data['vol_before'].apply(len) == 90]
    ...
    all = Dataset.from_pandas(pd.DataFrame(all))
    ...
    test_examples = dataset["train"]
    all_examples = dataset["all"]
    ...
    all_examples.set_transform(partial(transform_start_field, freq=freq))
    ...
    all_dataloader = create_test_dataloader(
        config=config,
        freq=freq,
        data=all_examples,
        batch_size=64,
    ...
    forecasts = np.concatenate(forecasts)[:, :, -1]
    
    errors = []
    all_seq_emb = [None] * len(all_examples)
    for item_id, ts in enumerate(all_examples):
        all_seq_emb[ts['item_id']] = forecasts[item_id]
    
        
    data['seq_emb'] = all_seq_emb
    
    data.to_pickle(args.output_path)
    
    data['seq_preds'] = data['seq_emb'].apply(lambda x: np.median(x))
        
    data['mse'] = (data['Y'] - data['seq_preds']) ** 2
    print('All MSE: {}'.format(data['mse'].mean()))
    print('Test MSE: {}'.format(data[data['date'].str[:4] == test_year]['mse'].mean()))
    print('Val MSE: {}'.format(data[data['date'].str[:4] == val_year]['mse'].mean()))
